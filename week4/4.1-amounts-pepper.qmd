...
jobs_clean |> 
  ...
  mutate(occupation = fct_reorder(occupation, total_earnings)) |> #reorder occupation column by values in another column
  ggplot(aes(x = occupation, y = total_earnings)) +
  geom_col() + # use when you want x and y 
  geom_text(aes(label = dollar(total_earnings)),
            hjust = 1.2, color = "white") +
  scale_y_continuous(labels = label_currency(scale = 00.1, suffix = "k")) +
  coord_flip()
...
jobs_clean |> 
  ...
  coord_flip() +
  #  #  scale_y_continuous(labels = label_currency(scale = 0.001, accuracy = 1), scale = 0.001, limits = c(0, 250000)) +
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k",)) +
   # use when you want x and y 
  geom_text(aes(label = dollar(total_earnings)),
            hjust = 1.2, vjust = -0.5, color = "black")
  
# scales, update formatting with dollar sign
```
...
ggplot(analysts, aes(x = year, y = occupation, fill = total_earnings)) + 
  ...
  coord_fixed() +
  scale_fill_viridis_c(labels = label_currency(scale = 0.001,
                                               suffix = "k")) +
