...
mko_clean |> 
  ggplot(aes(x = Temp_bot, y = month_name,
             fill = after_stat(x))) + # compute gradient of colors based on x after geom stat
...
mko_clean |> 
  ggplot(aes(x = Temp_bot, y = month_name,
             fill = after_stat(x))) + # compute gradient of colors based on x after geom stat
  geom_density_ridges_gradient(rel_min_height = -.01, scale = 3) +
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A")) 
...
ggplot(mko_clean, aes(month_name, Temp_bot)) + 
  geom_boxplot() 
```
...
# beware with jitter data (it adds redundant info I'm kind of confused, look how to fix below with outlier.shape())
ggplot(penguins, aes(x = species, y = body_mass_g)) +
  geom_boxplot() + 
  geom_jitter()
...
# violin plot & overlay with boxplot----
ggplot(mko_clean, aes(x = month_name, y = Temp_bot)) + 
  geom_violin() + 
